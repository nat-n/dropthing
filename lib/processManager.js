  ...
  remove: function handleRemovedFile(filename, Qs) {
    ...
    ['create', 'upload', 'publish'].forEach(function (queueName) {
      Qs[queueName] = Qs[queueName].filter(function (thing) {
        if (thing.filename === filename) {
          logger.log('verbose', 'Removed : "' + filename +
...
function newHeartBeat(Qs) {
  ...
  Qs.publish.forEach(function (thing) {
    ...
    if (thing.status === 'checking_thing') { thing.status = 'failed_publish'; }
  ...
  return function heartBeat() {
    ...
    Qs.publish = Qs.publish.filter(function (thing) {
      if (thing.failures > 15) {
        // Just ignore things that keep failing
        // The only known scenario for this is if connection is lost in the
        //  instant after publish request sent but before success response
        //  recieved, so the thing is marked failed_publish, but subsequent
        //  attempts to publish will all fail.
        return true;
      }
      switch (thing.status) {
        case 'uploaded':
        ...
        case 'failed_publish':
          thing.status = 'checking_thing';
          if (thing.failures > 3) {
            // Recover from error when publishing erroneously appears to fail
            thingiverse.getThing(thing.id, function (realThing) {
              console.log(realThing, realThing.is_published)
              if (realThing.is_published) {
                thing.failures = 0;
                thing.status = 'published';
              } else {
                thing.status = 'failed_publish';
